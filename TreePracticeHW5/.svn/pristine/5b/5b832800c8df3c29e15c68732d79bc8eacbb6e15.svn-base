...
public class BinarySearchTree {
	...
	public int getSumOfHeights() {
		...
		// calls to height()?
		int number = this.root.heights(true).sumHeights;
		return number;
	}
	...
	public class BinaryNode {
		...
		public TempVariables heights(boolean isRoot) {
			...
			if(this==NULL_NODE){
				TempVariables temp = new TempVariables(-1,0);
				return temp;
			}
...
//			System.out.println("RH  " + rightHeight);
			TempVariables temp1 = this.left.heights(true);
			TempVariables temp2 = this.right.heights(true);
			int currentNodeHeight = Math.max(temp1.nodeHeight, temp2.nodeHeight)+1;
			int currentSumHeights = temp1.sumHeights + temp2.sumHeights + currentNodeHeight;
			TempVariables nodeAt = new TempVariables(currentNodeHeight, currentSumHeights); 
			return nodeAt;
		}
